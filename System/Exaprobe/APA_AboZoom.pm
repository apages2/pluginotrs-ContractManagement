...
sub GetLine {
	...
	while ( my @Row = $Self->{DBObjectsage}->FetchrowArray() ) {
		...
		my $SQLAbo_Ligne = "SELECT TR_Abo_Ligne,TR_Type, TR_DateFin, TR_Type_Sage from APA_TR where TR_Abo_ID=? and TR_Abo_Ligne=?";
...
sub SaveTR {
	...
	my $Addrow  = "INSERT INTO APA_TR (TR_CustID, TR_Caff, TR_Type, TR_Option1, TR_Option2, TR_Perimetre, TR_Description, TR_DateDebut, TR_DateFin, TR_Commentaire, TR_Abo_Ligne, TR_IC, TR_Montant, TR_Abo_ID, TR_Type_Sage, TR_Option3, TR_Option4) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	if ($Type eq 'SO_COG-UO-1' || $Type eq 'SO_COG-UO-2' || $Type eq 'SO_COG-UO-3' || $Type eq 'SO_COG-UO-4' || $Type eq 'SO_COG-UO-5' || $Type eq 'SO_COG-UO-6') {
		...
		my $AddLog  = "UPDATE APA_TR SET TR_Log=? WHERE TR_Abo_ID=?";
...
sub RenewTR {
	...
	my $SQLAbonnement = "SELECT TR_Perimetre,TR_Commentaire,TR_Caff,TR_Type,TR_Description,TR_Option1, TR_Option2, TR_Option3, TR_Option4 from APA_TR where TR_Abo_ID=? AND TR_Abo_Ligne=?";
...
sub ApplyTR {
	...
	if ($Report== 1 || $Report== 2) {
		$Renew=$Option1*-1;
		my $SQLAbonnement = "SELECT TR_Option2,TR_ID,TR_Log from APA_TR WHERE TR_Abo_ID=? AND TR_Abo_Ligne=?";
	...
	my $Addrow  = "UPDATE APA_TR set TR_CustID=?, TR_Caff=?, TR_Type=?, TR_Option1=?, TR_Option2=?, TR_Perimetre=?, TR_Description=?, TR_DateDebut=?, TR_DateFin=?, TR_Commentaire=?, TR_IC=?, TR_Montant=?, TR_Type_Sage=?, TR_Option3=?, TR_Option4=?  WHERE TR_Abo_ID=? AND TR_Abo_Ligne=?";
