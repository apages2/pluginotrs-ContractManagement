...
package Kernel::System::DynamicFieldDatabase;
...
Kernel::System::DynamicFieldDatabase - module to interact with the databases
...
=item DatabaseSearchByConfig()

returns an array with the search results and additional meta information

    my @Result = $DynamicFieldDatabaseObject->DatabaseSearchByConfig(
...
=item DatabaseSearchByAttributes()

returns a hash with the search results

    my @Result = $DynamicFieldDatabaseObject->DatabaseSearchByAttributes(
        Config => $DynamicFieldConfig->{Config},
    ...
    for my $Key ( sort keys %{ $Param{Config}->{PossibleValues} } ) {
        ...
        if ( $Key =~ m/^\w+_(\d+)$/ ) {
            ...
            if ( !IsHashRefWithData( $PreparedPossibleValues->{$1} ) ) {
                ...
                $PreparedPossibleValues->{$1}->{$Key} = $Param{Config}->{PossibleValues}->{$Key};
            }
        }
    }
...
=item DatabaseSearchHistoricalData()
