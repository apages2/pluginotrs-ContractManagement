...
package Kernel::Modules::AdminDynamicFieldContactWithData;
...
sub _AddAction {
    ...
    if ( !$PossibleValues->{Name} || !$PossibleValues->{ValidID} ) {
        $Errors{'MandatoryFieldsServerError'} = 1;
    }

    # check, unify and precompute SortOrder, MandatoryFields and SearchableFields
    $Self->_ComputeFields(
        GetParam       => \%GetParam,
...
sub _ShowScreen {
    ...
    my %OrderNamesList;
    my $CurrentlyText = $Kernel::OM->Get('Kernel::Language')->Translate('Currently') . ': ';
    ...
    if ( $Param{MandatoryFieldsServerError} ) {
        $Param{MandatoryFieldsServerErrorMessage} = "Two keys 'Name' and 'ValidID' have to exist in possible values.";
        $Param{MandatoryFieldsServerError}        = 'ServerError';
    }

    # generate output
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AdminDynamicFieldContactWithData',
        Data         => {
            %Param,
