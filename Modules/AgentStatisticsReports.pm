...
sub AddScreen {
    ...
    # build output
    my $Output = $LayoutObject->Header( Title => 'Add New Statistics Report' );
    $Output .= $LayoutObject->NavigationBar();
    $Output .= $LayoutObject->Output(
...
sub AddAction {
    ...
    if ( !$StatsReportID ) {
        return $LayoutObject->ErrorScreen( Message => 'Could not create report.' );
    }
...
sub EditScreen {
    ...
    $Frontend{PhantomJSActive} = $Kernel::OM->Get('Kernel::Config')->Get('PhantomJS::Bin') ? 1 : 0;

    my %Format = %{ $Kernel::OM->Get('Kernel::Config')->Get('Stats::Format') || {} };

    my %FilteredFormats;
    for my $Key ( sort keys %Format ) {
        if ( $Frontend{PhantomJSActive} ) {
    ...
    my $StatsHook = $Kernel::OM->Get('Kernel::Config')->Get('Stats::StatsHook');
    ...
    if ($CronDefinition) {
        ...
        if ($CronNextEvent) {
            $Frontend{CronNextRunTimeStamp} = $Kernel::OM->Get('Kernel::System::Time')->SystemTime2TimeStamp(
                SystemTime => $CronNextEvent,
            );
            ...
            if ($CronLastRun) {
                $Frontend{CronLastRunTimeStamp} = $Kernel::OM->Get('Kernel::System::Time')->SystemTime2TimeStamp(
                    SystemTime => $StatsReport{Config}->{CronLastRun},
                );

                # The previous run might not yet have finished
                if ( $CronLastRun < $CronPreviousEvent ) {
                    $Frontend{CronNextRunTimeStamp} = $Kernel::OM->Get('Kernel::System::Time')->SystemTime2TimeStamp(
                        SystemTime => $CronPreviousEvent,
                    );
                }
    ...
    # build output
    my $Output = $LayoutObject->Header( Title => 'Edit Statistics Report' );
    $Output .= $LayoutObject->NavigationBar();
    $Output .= $LayoutObject->Output(
...
sub EditAction {
    ...
    if ( !$StatsReportID ) {
        return $LayoutObject->ErrorScreen( Message => 'Could not update report.' );
    }
...
sub ViewScreen {
    ...
    # build output
    my $Output = $LayoutObject->Header( Title => 'View Statistics Report' );
    $Output .= $LayoutObject->NavigationBar();
    $Output .= $LayoutObject->Output(
...
sub DeleteAction {
    ...
    if ( !$StatsReportID ) {
        return $LayoutObject->ErrorScreen( Message => 'Delete: Got no StatsReportID!' );
    }
...
sub StatsAddWidgetAJAX {
    ...
    my $PhantomJSActive = $Kernel::OM->Get('Kernel::Config')->Get('PhantomJS::Bin') ? 1 : 0;
    my %Format = %{ $Kernel::OM->Get('Kernel::Config')->Get('Stats::Format') || {} };
