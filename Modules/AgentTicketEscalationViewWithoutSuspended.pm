...
# --
# $origin: otrs - 471b7844e7a3eaf9b71fdef7ae277bf7790ed131 - Kernel/Modules/AgentTicketEscalationView.pm
# --
...
package Kernel::Modules::AgentTicketEscalationViewWithoutSuspended;
# ---
...
# OTRSEscalationSuspend
# ---
    my %SearchStatesWithoutSuspend;
    my %List = $Kernel::OM->Get('Kernel::System::State')->StateList(
        UserID => 1,
    );
    my @States = @{ $ConfigObject->Get('EscalationSuspendStates') };
    my @StateIDs;
    for my $StateListID ( sort keys %List ) {
        my $Hit;
        STATE:
        for my $State (@States) {
            if ( lc( $List{$StateListID} ) eq lc($State) ) {
                $Hit = 1;
                last STATE;
            }
        }
        if ( !$Hit ) {
            push @StateIDs, $StateListID;
        }
    }
    %SearchStatesWithoutSuspend = (
        StateIDs => \@StateIDs,
    );
...
#        Action            => 'AgentTicketEscalationView',
        Action            => 'AgentTicketEscalationViewWithoutSuspended',
# ---
...
#        TitleName  => Translatable('Ticket Escalation View'),
        TitleName => Translatable('Ticket Escalation View Without Suspend State'),
